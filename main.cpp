...
void auxFunctions() 
{                                           // voltage meter function
  float voltage;
  long x;
  if (digitalRead(8) == LOW) 
  {                                         // if SELECT button pushed, measure battery voltage
  ...
  {                                         // if DOWN botton pushed, 50V range
    analogReference(INTERNAL);
...
{                                           // get various information from wave form
  ...
  {                                               // amplitude of the waveform smaller than the specified value
    trigSync = false;                             // set Unsync display flag
  }
  freqDuty();
}
...
void saveEEPROM() 
{                    // Save the setting value in EEPROM after waiting a while after the button operation.
